...
|---|---|---|
| R0 | Worth recording at all | Q1 (reach) and Q2 (cost of reversal) from "Before writing", applied to the finished record -- and to **each assertion R1's split produces**, not only to the record as a whole. A genuine decision in one half does not carry the other half through; a record whose title joins two clauses is the ordinary case of this, not the exception. A convention, a local detail, a reversible default or a piece of work planning fails here while every rule below reads `ok` -- that combination is the whole reason this row exists. Two shapes sound architectural and still fail. **Status quo:** the assertion is that things stay as they are. The chosen option is what the project already does, so reversing it costs nothing because nothing was taken and Q2 fails however weighty the subject sounds -- "X stays as it is", "no Y is changed", or a title that reads as a state of affairs rather than a choice. That an alternative was discussed and turned down does not save it: what the record then holds is the discussion, and what it decided is that nothing happens. **Instance list:** an enumeration of which components, modules or contexts exist, or of which one holds which, is a building-block view, not a decision. If a decision is in there at all, it is the rule the list was cut by -- and that rule is its own record, one that stays true as the enumeration ages. |
...
| R3 | No external references | `adr_check`'s `Suspicions` already flag a tracker-looking reference (`#123`); a reader confirms the flag is actually one (not, say, a decision or option count that happens to look like it) and decides how to rewrite the record so it stands on its own without it. The same defect in a different guise, which no tool flags: the record narrating **its own drafting history** -- what an earlier draft of this record wanted, why that was dropped, what has been clarified since. The reader has none of those, and the record is not about its own editing; the context states the situation the decision was taken in, not how its text got there. |
