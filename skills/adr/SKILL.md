...
|---|---|---|
| R0 | Worth recording at all | Q1 (reach) and Q2 (cost of reversal) from "Before writing", applied to the finished record. A convention, a local detail, a reversible default or a piece of work planning fails here while every rule below reads `ok` -- that combination is the whole reason this row exists. Two shapes sound architectural and still fail. **Status quo:** the chosen option is what the project already does, no alternative was ever live, and reversing it would cost nothing because nothing was taken -- Q2 fails however weighty the subject. A title that reads as a state of affairs rather than a choice ("X is Y", "the system has Z") is the tell; the content describes the system and belongs to the resources that describe it, not to a decision. **Instance list:** a record enumerating which components, modules or contexts exist is a building-block view, not a decision. If a decision is in there at all, it is the rule the list was cut by -- and that rule is its own record, one that stays true as the enumeration ages. |
