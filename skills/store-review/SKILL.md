...
5. **Report the gaps as gaps.** The reader-level review modes are unevenly
   developed, and a mode existing is not the same as it reaching everything.
   This skill covers what they carry and says plainly, in the coverage table
   of the report, what each mode does *not* reach -- and, for a resource type
   that has no mode at all, that it was not reviewed. Never quietly left out,
   and never given an improvised rule set invented for this run. Every type
   the store holds today has a mode; this rule is what keeps the next one
   that arrives without one from passing as reviewed.
...
| `bc_list`, `bc_get` | Reader | Every Bounded Context, each with its recorded `ContextRelationship` edges shown inline by both calls -- the same read serves the Bounded Context reviewer and the context-relationship one. |
...
| Bounded Context | The review mode of `/arknet:bc-audit` -- rules B1-B6, one row per recorded context, plus its two set-wide findings | The mode re-tests a recorded boundary; it does not look for a boundary the store is *missing*. That is the same skill's candidate mode, an elicitation, and it stays outside this pass |
| Context relationship | The review mode of `/arknet:context-map` -- rules C1-C4, one row per recorded edge, plus type and justification uniformity across the map | A pair with no edge recorded has no row here: the reader cannot tell an unrelated pair from an unrecorded one, which is `/arknet:bc-audit`'s B2 for the context and an elicitation for the pair |
| Actor, Role | The per-actor and per-role checklists of `/arknet:req-interview`, where both types are elicited | Neither checklist has a mechanical backstop for its central question: no tool compares two actors or two roles, so a duplicate is found by reading or not at all |
...
not look at. Every type in the table has one today; rule 5 stands for the next
resource type that arrives without one.
